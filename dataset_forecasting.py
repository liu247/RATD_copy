...
class Dataset_Electricity(Dataset):
    ...
    def __read_data__(self):
        ...
        self.reference=torch.clamp(self.reference,min=0, max=17885) # make the tensor in range [min, max]
...
def get_dataloader(device, batch_size=8):
    dataset = Dataset_Electricity(root_path='./data/ts2vec',data_path="electricity_2012_hour.csv",flag='train',size=[96,0,168, 320]) # [96,0,168, 321]
    train_loader = DataLoader(
        dataset, batch_size=batch_size, shuffle=1)
    valid_dataset = Dataset_Electricity(root_path='./data/ts2vec',data_path="electricity_2012_hour.csv",flag='val',size=[96,0,168,320])
    valid_loader = DataLoader(
        valid_dataset, batch_size=batch_size, shuffle=0)
    test_dataset = Dataset_Electricity(root_path='./data/ts2vec',data_path="electricity_2012_hour.csv",flag='test',size=[96,0,168,320])
